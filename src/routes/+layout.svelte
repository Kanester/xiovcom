...
{:else}
	<header class="header">
		...
		{#if $user}
			<div id="profile">
				<button class="avatar-btn" onclick={() => toggle('dropdown')} aria-expanded={dropdownOpen} aria-controls="dropdown">
					<img src={$user.photoURL ?? '/default-avatar.svg'} alt="avatar" class="avatar" />
				</button>
	...
	{#if signInOpen}
		<div id="authBackdrop" onclick={closeModal} onkeydown={(e) => (e.key === 'Escape' || e.key === 'Enter') && closeModal()} role="presentation" transition:fade>
			<div
				id="authModal"
				tabindex="0"
				role="dialog"
...
#authModal {
	...
	#provider {
		...
		button {
			...
			svg {
				/* reusing your icon styles or mixins if you have them */
			}
